...
const newTheatreBody = async (req,res,next)=>{
    try{
        ...
        if (!req.body.showTypes) {
            return res.status(400).send({
                message: "Failed ! Movie showTypes are not provided"
            });
        }else{
        ...
        }else if (typeof req.body.ticketPrice !== "number" || req.body.ticketPrice < 0){
            return res.status(400).send({
                message: "Failed ! Number of seates is not in correct format (Number) and there is not negitive number !"
            });
        }
...
const editTheatreBody = (req,res,next)=>{
    try{
        ...
        if (req.body.ticketPrice && typeof req.body.ticketPrice !== "number"){
            return res.status(400).send({
                message: "Failed ! ticket is not in correct format (Number) or is not a negative number"
            });
        }
        next();
    }catch{
