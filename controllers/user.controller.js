...
exports.findAll = async (req,res)=>{
    ...
    try{
        ...
        console.log("___ Error while fetching all user's data ___ ", err.message);
        res.status(500).send({
...
exports.findByUserId = async (req,res)=>{
    try{
        ...
        console.log("___ Error while searching for the user ___", err.message);
        res.status(500).send({
            message : "Internal server error while fetching data"
        })
    }
...
exports.updateUser = async (req,res)=>{
    try{
        ...
        console.log(`___${updatedUser.userType} ${updatedUser.name} data updated ___`);
        res.status(200).send(objectConverter.singleUserResponse(updatedUser));

    }catch(err){
        console.log("___ Error while updating user data ___ ", err.message);
        res.status(500).send({
